...
abstract class DataLayer
{
  ...
  public function innerVenda(?string $terms = null, ?string $params = null, string $columns = "*"): DataLayer
  {
    if ($terms) {
      $this->statement = "SELECT {$columns} FROM {$this->entity} 
      inner join rota on rota.id = venda.rota_id
      WHERE {$terms}";
      parse_str($params, $this->params);
      return $this;
    }

    $this->statement = "SELECT *, venda.id as id_venda FROM {$this->entity} 
        inner join rota on rota.id = venda.rota_id
        where venda.status = 'A' AND venda.login_id = {$_SESSION['user']}
        ";
    return $this;
  }
  ...
  public function innerMotorista(?string $terms = null, ?string $params = null, string $columns = "*"): DataLayer
  {
    if ($terms) {
      $this->statement = "SELECT {$columns} FROM {$this->entity} 
      inner join login on login.id = motorista.login_id
      WHERE {$terms}";
      parse_str($params, $this->params);
      return $this;
    }

    $this->statement = "SELECT *, motorista.ativo as mot_ativo FROM {$this->entity} 
        inner join login on login.id = motorista.login_id";
    return $this;
  }
